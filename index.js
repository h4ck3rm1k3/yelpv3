...
const proxyUrl = window.location.protocol + "//"  + window.location.hostname + (window.location.port ? ':' + window.location.port : '') + '/yelp/v3';
...
class Yelpv3 {
    ...
    get(resource, params) {
        ...
        return this.getAccessToken().then((token) => {
            return request({
                uri: proxyUrl + resource + jsonToQueryString(params),
                headers: {
